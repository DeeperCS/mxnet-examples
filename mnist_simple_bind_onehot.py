...
simple_bind example using onehot label
"""
...
# bind label with size (batch_size, 10)
executor = network.simple_bind(ctx=mx.gpu(0), data_mnist=(batch_size,784), label_mnist=(batch_size, 10))
...
# variable for intermediate prediction results
...
for epoch_i in range(nb_epoch):
    ...
    for batch_i in range(batch_round):
        ...
        for idx, key in enumerate(args):
            if is_param_name(key):
                ### Notice to divide 'batch_size' !!!
                updater(idx, grads[key]/batch_size, args[key])
            
        train_acc += np.sum(pred_prob.asnumpy().argmax(axis=1)==label.argmax(axis=1))*1.0 / batch_size
    ...
    for batch_i in range(batch_round):
        ...
        pred_prob[:] = executor.outputs[0]
        # print pred_prob.asnumpy().argmax(axis=1) 
        val_acc += np.sum(pred_prob.asnumpy().argmax(axis=1)==label.argmax(axis=1))*1.0 / label.shape[0]
        # logging.info("Finish training iteration %d" % batch_i)
    val_acc /= batch_round
    logging.info("Epoch {} Validation Acc {:.4f}".format(epoch_i, val_acc))
    if best_val_acc<val_acc:
        best_val_acc=val_acc
        mx.callback.save_checkpoint('best_model', 0, network, executor.arg_dict, executor.aux_dict)
